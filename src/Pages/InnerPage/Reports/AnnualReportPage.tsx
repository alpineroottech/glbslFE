...
const AnnualReportPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchAnnualReports = async () => {
      try {
        setLoading(true);
        const response = await reportsService.getReportsByType('annual');
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          ...
          url: `${window.location.origin}/reports/annual/${report.slug}`,
        });
      } catch (error) {
  ...
  return (
    <section className="">
      <BreadCrumb title={t('submenu.annual_report')} home={"/"} />
      ...
      <div className="bg-whiteSmoke dark:bg-lightBlack py-20 2xl:py-[120px]">
        <div className="Container">
          ...
          {loading && (
            <div className="flex justify-center items-center py-20">
              <div className="text-center">
                ...
                <p className="mt-4 text-gray dark:text-lightGray">Loading annual reports...</p>
              </div>
            </div>
          )}
          ...
          {!loading && !error && reports.length === 0 && (
            <div className="flex justify-center items-center py-20">
              <div className="text-center">
                <p className="text-gray dark:text-lightGray">No annual reports available at the moment.</p>
              </div>
            </div>
          )}
