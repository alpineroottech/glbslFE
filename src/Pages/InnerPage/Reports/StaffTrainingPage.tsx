...
const StaffTrainingPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchStaffTrainingReports = async () => {
      try {
        setLoading(true);
        const response = await reportsService.getReportsByType('staff-training');
        setReports(response.data || []);
      } catch (err) {
        setError('Failed to load staff training reports');
        console.error('Error fetching staff training reports:', err);
      } finally {
        setLoading(false);
      }
    };

    fetchStaffTrainingReports();
  }, [language]);
      ...
      <div className="bg-whiteSmoke dark:bg-lightBlack py-20 2xl:py-[120px]">
        <div className="Container">
          ...
          {loading && (
            <div className="flex justify-center items-center py-20">
              <div className="text-center">
                ...
                <p className="mt-4 text-gray dark:text-lightGray">Loading staff training reports...</p>
              </div>
            </div>
          )}
          ...
          {!loading && !error && reports.length === 0 && (
            <div className="flex justify-center items-center py-20">
              <div className="text-center">
                <p className="text-gray dark:text-lightGray">No staff training reports available at the moment.</p>
              </div>
            </div>
          )}
